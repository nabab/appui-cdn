...
foreach ( $files as $f ){
  foreach ( file($f) as $ln => $c ){
    while ( ($pos = Str::pos($c, 'bbn.')) !== false ){
      $c = Str::sub($c, $pos + 4);
      if ( $idx = Str::pos($c, '.') ){
        $var = Str::sub($c, 0, $idx);
        $c = Str::sub($c, Str::len($var) + 1);
        if ( \in_array($var, $vars) && ($res = preg_match('/[^A-z0-9_]/', $c, $matches, PREG_OFFSET_CAPTURE)) ){
          $i = $matches[0][1];
          $key = Str::sub($c, 0, $i);
          $c = Str::sub($c, Str::len($key) + 1);
          if ( !isset($bbn[$key]) ){
            $bbn[$key] = [
              'name' => $key,
              'ns' => $var,
...
foreach ( $bbn as $k => $v ){
  $new = $k;
  while ( $idx = Str::pos($new, '_') ){
    $new = Str::sub($new, 0, $idx).
      strtoupper(Str::sub($new, $idx+1, 1)).
      Str::sub($new, $idx+2);
  }
  if ( $v['ns'] === 'var' ){
    $new = '_'.$new;
  ...
  else if ( ($v['ns'] === 'fn') && (Str::pos($new, 'default') === 0) ){
    $new = '_fn' . Str::sub($new, 7, -8);
  }
